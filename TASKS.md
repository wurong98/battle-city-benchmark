...
## [P1] 射击系统与地形破坏
- [ ] **Task 1.1: Bullet 实体与炮口发射**
  - 创建 `src/game/entities/Bullet.ts`。
  - 根据坦克当前朝向从炮口位置生成子弹，限制同屏单发（升级后支持双发）。
- [ ] **Task 1.2: 子弹推进系统 (BulletSystem)**
  - 实现子弹高速匀速移动与越界销毁。
- [ ] **Task 1.3: 子弹与瓦片碰撞及 4-bit 砖墙破坏**
...
- [ ] **Task 3.1: 基地实体 (Eagle Base)**
  - 地图底端正中央放置基地实体与防护砖墙。
  - 基地中弹后变为残骸，触发判定。
- [ ] **Task 3.2: 爆炸特效系统 (EffectSystem)**
  - 创建 `src/game/entities/Explosion.ts`，多阶段爆炸动画帧循环与自销毁。
- [ ] **Task 3.3: 胜负与关卡规则 (GameRuleSystem)**
  - 基地被毁或玩家生命耗尽 -> `GAME_OVER`。
  - 关卡敌人全灭 -> `STAGE_CLEAR` -> 结算并载入下一关。
- [ ] **Task 3.4: 低频状态总线与 React HUD / Overlay 呈现**
  - 创建 `src/store/gameStore.ts` (Zustand)。
...
## [P4] 经典特性还原与细节打磨
- [ ] **Task 4.1: 草地遮蔽 (Z-Index Layering) 与冰面/水流效果**
  - Grass 移至渲染最顶层（覆盖坦克与子弹）。
- [ ] **Task 4.2: 敌人兵种分化**
  - 实现 Basic, Fast, Power, Armor 4 类敌人的差异化速度与 HP。
- [ ] **Task 4.3: 道具拾取系统 (Power-Up)**
  - 闪光敌人掉落头盔（无敌）、定时器（冻结敌人）、铲子（铁墙护基地）、手雷（全屏击杀）等。
- [ ] **Task 4.4: 像素贴图 (Spritesheet) 与音效集成**
  - 载入经典像素贴图资产，替换纯色矩形。
  - 集成 Web Audio / Howler，支持射击、爆炸、移动背景音。
